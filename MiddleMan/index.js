    ...
    try {
        if (interaction.isChatInputCommand()) {
            if (interaction.commandName === "panel") {
                ...
                if (selectedChannel) {
                    data.config.panelChannelId = selectedChannel.id;
                    saveData();
                }

                const channel = selectedChannel || interaction.guild.channels.cache.get(cfg("panelChannelId")) || interaction.channel;
            ...
            if (interaction.commandName === "crosstrade") {
                ...
                if (!cfg("middlemanRoleId") || !cfg("logChannelId")) {
                    return interaction.reply({ content: "The Middleman role and logs channel must be configured before using this command.", ephemeral: true });
                }
        ...
        if (interaction.isButton()) {
            ...
            if (agreeMatch) {
                ...
                if (!trade || trade.channelId !== interaction.channel.id || trade.status === "confirmed") {
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId === "middleman_modal") {
                ...
                if (cfg("middlemanRoleId")) {
                    overwrites.push({
                        id: cfg("middlemanRoleId"),
            ...
            if (interaction.customId === "crosstrade_first") {
                ...
                const ticket = data.tickets[interaction.channel.id];
                if (!ticket || ticket.status !== "open") {
                    return interaction.reply({ content: "This Cross Trade must be started inside an open middleman ticket.", ephemeral: true });
                }

                if (!([ticket.requesterId, ticket.traderId].includes(person1) && [ticket.requesterId, ticket.traderId].includes(person2))) {
                    return interaction.reply({ content: "Person 1 and Person 2 must be the two traders in this ticket.", ephemeral: true });
                }
